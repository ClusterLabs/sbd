...
If you specify the I<create> command, B<sbd> will write a metadata header
...
This command will make B<sbd> start in daemon mode. It will constantly monitor
the message slot of the local node for incoming messages, reachability, and
optionally take Pacemaker's state into account.

B<SBD> B<must> be started on boot before the cluster stack! See below
...
protection against B<sbd> crashing.

If the Pacemaker integration is activated, B<sbd> will B<not> self-fence
...
Watchdog timeout to set before crashdumping. If B<SBD> is set to crashdump
...
to check if B<SBD> is seeing the device. Note that this could overwrite a
...
This will make the B<sbd> daemon exit cleanly on the target. You should
...
You should not set this manually; B<sbd> will clear the message slot
...
and perhaps multipath alias names to ease management, use the B<sbd create>
...
start-up scripts must handle starting or stopping B<sbd> as required
...
disabled monitor processes, you can send a I<SIGUSR1> to the B<SBD>
